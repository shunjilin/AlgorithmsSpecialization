...
namespace SCC {
    ...
    using LeaderToSCCSize = std::unordered_map<unsigned, unsigned>;
    LeaderToSCCSize getSCCSizes(const NodeToLeader& node_to_leader) {
        LeaderToSCCSize leader_to_scc_size;
        for (auto leader : node_to_leader) {
            ++leader_to_scc_size[leader];
        }
        return leader_to_scc_size;
    }

    LeaderToSCCSize computeSCC(std::size_t n_nodes,
        ...
        return getSCCSizes(node_to_leader);   
    }
